...
def tune_torch_compile(
    *,
    ...
    """Decide whether to use torch.compile for training, caching the decision per (device, cfg, shapes)."""
    ...
    if cache_path:
        ...
        if isinstance(cached, dict) and "enabled" in cached:
            try:
                ...
                plan = TrainCompilePlan(enabled=want, mode=mode, warmup=warmup, iters=iters, hysteresis=hysteresis)
                if want and enabled_default:
                    try:
                        return torch.compile(model, mode=mode), plan
                    except Exception:
                        return model, TrainCompilePlan(enabled=False, mode=mode, warmup=warmup, iters=iters, hysteresis=hysteresis)
                return model, plan
            except Exception:
                pass
    ...
    try:
        ...
        try:
            ...
            plan = TrainCompilePlan(enabled=False, mode=mode, warmup=warmup, iters=iters, hysteresis=hysteresis)
            if cache_path:
                try:
                    set_cache_entry(str(cache_path), section="train_compile", key=key, value={"enabled": False, "ts": float(time.time())})
                except Exception:
                    pass
            return model, plan

        tok_s_comp = _bench_train_tok_s(
            device=device,
            model=compiled,
            get_batch=get_batch,
            batch_size=batch_size,
        ...
        if cache_path:
            try:
                set_cache_entry(
                    ...
                    key=key,
                    value={"enabled": bool(want), "tok_s_base": float(tok_s_base), "tok_s_comp": float(tok_s_comp), "ts": float(time.time())},
                )
            except Exception:
                pass
...
def tune_batch_by_seq(
    *,
    ...
    """Tune (batch_size, grad_accum) for each seq_len, with caching and RNG preservation."""
    ...
    try:
        ...
        for s in seq_lens:
            ...
            if verbose:
                if auto:
                    print(f"[selfopt][train] tuning seq_len={s} (auto gbs)")
                else:
                    print(f"[selfopt][train] tuning seq_len={s} target_gbs={target_gbs}")

            # Auto mode: probe for the best micro-batch at ga=1 by doubling until OOM.
            if auto:
                ...
                while bs_try <= 4096:
                    ...
                    bs_try *= 2
                # Also add a few intermediate points for smoother peak detection.
                if candidates:
                ...
                for bs_try, ga_try in uniq:
                    ...
                    try:
                        ...
                        for _ in range(iters):
                            ...
                            peak = max(peak, float(m.get("mps_mem_driver_bytes", 0.0) or 0.0), float(m.get("cuda_mem_reserved_bytes", 0.0) or 0.0))
                        device_synchronize(device)
                        dt = time.perf_counter() - t0
                        tok_s = float(tok / max(dt, 1e-9))
                        last_ok = max(last_ok, int(bs_try))
                    except Exception as e:
                        ok = False
                        tok_s = 0.0
            ...
            for bs_try in bs_list:
                ...
                # Time forward+backward only; do not step optimizer (keeps model weights unchanged).
                ok = True
                tok = 0
                peak = 0.0
                try:
