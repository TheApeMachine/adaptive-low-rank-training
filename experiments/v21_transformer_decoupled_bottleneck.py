...
class ModelConfig:
    ...
    attn_mode: Literal["standard", "bottleneck", "decoupled"] = "bottleneck"
...
class DecoupledBottleneckAttention(nn.Module):
    ...
    def _shape(self, x: torch.Tensor, head_dim: int) -> torch.Tensor:
        # (B,T,H*hd)->(B,H,T,hd)
        B, T, D = x.shape
        return x.view(B, T, self.H, head_dim).transpose(1, 2).contiguous()
        ...
        *,
        ...
        if cfg.attn_mode in ("standard", "bottleneck"):
            ...
            if attn_mask is not None:
                ...
                # Fallback internal masking if not provided (legacy behavior, though GPT.forward now provides it)
...
class GPT(nn.Module):
    ...
    def forward(self, idx: torch.Tensor, *, caches: Optional[List[Any]] = None, pos_offset: int = 0) -> Tuple[torch.Tensor, Optional[List[Any]]]:
        ...
        if caches is None:
            ...
            # KV-cache prefill / chunked decode: ensure causality inside the new chunk.
            prev_len = caches[0].pos if (caches and len(caches) > 0) else 0
            total = prev_len + T
            q_pos = torch.arange(prev_len, total, device=idx.device).view(T, 1)
            k_pos = torch.arange(total, device=idx.device).view(1, total)
            attn_mask = (k_pos <= q_pos).view(1, 1, T, total)
        ...
        *,
        ...
        for _ in range(self.cfg.n_layer):
            if self.cfg.attn_mode == "decoupled":
                ...
                dim = self.cfg.d_model if self.cfg.attn_mode == "standard" else self.cfg.attn_dim
                caches.append(
                    LayerKVCache(
                        batch_size=B,
                        max_seq_len=max_seq,
...
def main() -> None:
    ...
    ap.add_argument("--attn-mode", type=str, default="bottleneck", choices=["standard", "bottleneck", "decoupled"])
