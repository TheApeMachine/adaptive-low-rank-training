...
\begin{itemize}
    \item \textbf{Up to 168$\times$ KV-cache reduction} in a fixed-rank long-context scenario via dimension reduction plus 4-bit quantization (Section~2.6)
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.95\textwidth]{assets/convergence_wikitext.png}
...
\centering
\small
\caption{FineWeb-Edu Validation Loss (100M tokens, 1024 context, 6000 steps; v29 suite)}
\label{tab:fineweb}
\begin{tabular}{@{}lccccc@{}}
\toprule
\textbf{Model} & \textbf{Attn Config} & \textbf{Params} & \textbf{Val Loss} & \textbf{Val PPL} & \textbf{$\Delta$} \\
\midrule
Standard Baseline & $d = 512$ & 139.8M & 6.326 & 559.0 & --- \\
GQA (kv=2) & 12Q/2KV & 128.0M & 6.302 & 545.9 & -2.4\% \\
Bottleneck (rank 144) & $d_{\text{attn}}=144$ & 116.8M & 6.127 & 458.1 & -18.0\% \\
\textbf{Decoupled 48/96} & $d_{\text{sem}}{=}48, d_{\text{geo}}{=}96$ & \textbf{116.8M} & \textbf{5.862} & \textbf{351.5} & \textbf{-37.1\%} \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Scaling Observation.}
Surprisingly, on FineWeb-Edu (100M tokens), the Decoupled Bottleneck model \textit{outperformed} the Standard Baseline, a parameter-matched GQA model, and a matched-rank Bottleneck baseline, achieving a \textbf{37\% reduction in perplexity}. This contradicts the common assumption that compression must hurt performance. We hypothesize that standard high-dimensional attention can be sufficiently over-parameterized to hinder optimization at this scale, whereas explicitly reducing the interaction rank (and disentangling content from positional geometry) acts as a regularizer that forces the model to learn more robust features.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.95\textwidth]{assets/m4max_big_convergence.png}
\caption{Validation loss curves on FineWeb-Edu (100M tokens). Under the same training setup, the Decoupled Bottleneck model converges to the lowest validation loss, outperforming both the standard baseline and GQA.}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.8\textwidth]{assets/m4max_big_comparison_bar.png}
\caption{Final validation loss comparison (FineWeb-Edu, v29). Lower is better. Decoupled 48/96 achieves the best loss, outperforming both the standard baseline and a parameter-matched GQA baseline (kv=2).}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.8\textwidth]{assets/m4max_big_pareto.png}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\textwidth]{assets/memory_footprint.png}
...
We have demonstrated that attention in Transformers contains significant redundancy. On FineWeb-Edu (100M tokens), the Decoupled Bottleneck architecture \textbf{surpassed} the standard baseline, a parameter-matched GQA baseline, and a matched-rank Bottleneck baseline, suggesting that for many practical scales, standard attention is not just inefficient but suboptimal.
